...
int number_of_points_series = 50;
int serial1_value = 0;
...
void reset_chart(){
  // Reset chart.
  ser_Temp = lv_chart_add_series(ui_Chart1, lv_palette_main(LV_PALETTE_YELLOW), LV_CHART_AXIS_PRIMARY_Y);
  ser_Hum = lv_chart_add_series(ui_Chart1, lv_palette_main(LV_PALETTE_BLUE), LV_CHART_AXIS_SECONDARY_Y);
  for (int i = 0; i < number_of_points_series; i++) {
    if (ui_Chart1 && ser_Temp) {
        lv_chart_set_next_value(ui_Chart1, ser_Temp, 0);
        lv_chart_set_next_value(ui_Chart1, ser_Hum, 0);
    } else {
        Serial.println("Chart or series is NULL!");
    }
  } 
...
{
  Serial.begin(115200);
  //Serial1.begin(115200, SERIAL_8N1, 44, 43); // Initialize Serial1 on GPIO 44 (RX) and GPIO 43 (TX)
...
void loop()
{
    ...
    /* if (Serial.available()) {
      ...
      serial1_value = str.toInt();
      lv_chart_set_next_value(ui_Chart1, ser_Hum, serial1_value);
    } */
